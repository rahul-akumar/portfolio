<template>
  <div class="min-h-screen bg-black text-white">
    ...
    <div class="relative z-10">
      <NavBar />
      <main class="relative">
        <section class="w-6xl mx-auto px-6 sm:px-8 py-16">
          <SectionHeader
            title="Case"
            highlight="Studies"
...
<img :src="post.cover" alt="" class="w-full h-48 object-cover group-hover:scale-[1.02] transition-transform" />
              ...
              <div class="mt-3 text-white/50 text-sm" v-if="post.date">{{ formatDate(post.date) }}</div>
            </NuxtLink>
          </div>

          <div v-else class="text-center text-white/70 py-20">No case studies yet.</div>
        </section>
      </main>
    </div>
  </div>
