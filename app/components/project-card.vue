...
<template>
  <div
    class="group project-card relative border border-white/10 backdrop-blur-3xl rounded-4xl p-8 sm:p-12" :class="[gradientClasses, { 'animate-fade-in-up': isVisible }]"
  >
      ...
      <div class="flex flex-col gap-4 md:w-md mt-8 lg:mt-0 ">
        ...
        <div class="flex flex-wrap gap-12 mt-6">
          <div v-if="project.metrics?.engagement">
            <p class="text-white/75 text-sm mb-1">
              {{ project.metrics.engagement.label || 'Engagement' }}
            </p>
            <h3 class="text-white text-2xl lg:text-3xl font-bold">
              {{ project.metrics.engagement.value }}
            </h3>
          </div>
          <div v-if="project.metrics?.satisfaction">
            <p class="text-white/75 text-sm mb-1">
              {{ project.metrics.satisfaction.label || 'User Satisfaction' }}
            </p>
            <h3 class="text-white text-2xl lg:text-3xl font-bold">
          ...
          <div v-if="project.metrics?.other">
            <p class="text-white/75 text-sm mb-1">
              {{ project.metrics.other.label }}
            </p>
            <h3 class="text-white text-2xl lg:text-3xl font-bold">
              {{ project.metrics.other.value }}
            </h3>
          </div>
        </div>
      </div>
