<template>
  ...
  <div v-else class="text-white/50 text-sm">No tech stack listed.</div>
</template>
...
const resolvedItems = computed(() => {
  return (props.stack || [])
    .map((raw) => {
      ...
      if (key.includes('nuxt')) return { key, ...iconMap.nuxt }
      if (key.includes('vue')) return { key, ...iconMap.vue }
      if (key.includes('react')) return { key, ...iconMap.react }
      if (key.includes('next')) return { key, ...iconMap.next }
      if (key.includes('tailwind')) return { key, ...iconMap.tailwind }
      if (key.includes('typescript') || key === 'ts') return { key, ...iconMap.typescript }
      if (key.includes('javascript') || key === 'js') return { key, ...iconMap.javascript }
      if (key.includes('node')) return { key, ...iconMap.node }
      if (key.includes('postgres')) return { key, ...iconMap.postgres }
      if (key.includes('mongo')) return { key, ...iconMap.mongodb }
      if (key.includes('vercel')) return { key, ...iconMap.vercel }
